...
We want now to define anti-matroids and investigate the connection with convex geometries. 

\begin{defn}
    An anti-matroid is a set system $(\mathcal{E}, \mathbb{S})$ such that:
    \begin{enumerate}
        \item $\emptyset \in \mathbb{S}$
        \item for every $A, B \in \mathbb{S}$ such that $ B \not\subset A$, there exists $e \in B \setminus A$ such that $A \cup \{ e \} \in \mathbb{S}$.
    \end{enumerate}
...
\begin{es} A simple example of anti-matroid is:
    ...
    \end{tikzpicture} \end{center}
    \caption{The Hasse diagram of an anti-matroid.}
    \label{fig:antimat}
    \end{figure}
\end{es}
Let's define the dual notion of set system.
...
It's possible to prove that a set system $(\mathcal{E}, \mathbb{S})$ is an anti-matroid if and only if the complementary set system $(\mathcal{E}, \mathbb{S}^c)$ is a convex geometry. Thus we can alternately define convex geometries as the dual notion of anti-matroid.
...
We introduce now the supersolvable version of anti-matroids, convex geometries and closure operator, and investigate their relation with building sets.

\begin{defn}
    Let $(\mathcal{E}, \mathbb{S})$ be a set system. Let $<$ be a total order on $\mathcal{E}$. We say that $\mathbb{S}$ is a supersolvable anti-matroid if:
    \begin{enumerate}
        \item $\emptyset \in \mathbb{S}$;
        \item for every $A, B \in \mathbb{S}$ with $B \not\subseteq A$, for $e = \min_{<}(B \setminus A)$ we have $A \cup \{ e \} \in \mathbb{S}$.
    \end{enumerate}
...
Proposition \ref{prop:buildsetlinearext} is a direct consequence of Definition \ref{def:scg} and next proposition which is, in fact, an extension of the latter.
...
\begin{proof}
    We already proved that if $g \in G$, and $N \subset \mathcal{G}$ is a $\mathcal{G}$-nested set then $g(N)$ is a $\mathcal{G}$-nested set, and since $g \in G$ are lattice automorphisms we have that $g(\vee N) = \vee g(N)$.
...
The space $\mathbb{P}(V)$ parametrizes 1-dimensional subspaces of $V$.
...
We give here some notions about blowups in $\mathbb{C}^n$ in order to give the second definition of the De Concini-Procesi wonderful model.
...
\begin{es}
    ...
    and the result is an open Möbius band. The exceptional divisor, $D_{123} = \sigma^{-1}(\{0\}) \simeq \mathbb{RP}^1$, intersects transversally wit hthe proper trasnforms $D_{ij}$ of the hyperplanes $H_{ij}$ for $1 \le j < j \le 3$.
\end{es}
...
\begin{thm}
    \begin{enumerate}
        ...
        \item The complement of $\pi^{-1}(\mathcal{M}(\mathcal{A}))$ in $Y_{\mathcal{A}}$ is a divisor with normal crossings; the irreducible componnents are the proper transforms $D_X$ of intersections $X$ in $\mathcal{L}(\mathcal{A})$,
        ...
        \mathcal{L}(\mathcal{A})_{\ge \hat{0}}$ intersects if and only if $\mathcal{S}$ is a linearly ordered subset in  $\mathcal{L}(\mathcal{A})_{\ge \hat{0}}$. If we think about $Y_{\mathcal{A}}$ as stratified by the irreducible components of the normal crossing divisor and their intersections, then the poset of strata coincides with the face poset of the order complex $\Delta(\mathcal{L}_{\ge \hat{0}})$.
    \end{enumerate}
\end{thm}

In Chapter 1 we studied the geometry of the family of building set in a lattice $\mathcal{L}$.
Here we want to briefly investigate the information encoded in the set complex $\mathcal{N}(\mathcal{L}, \mathcal{G})$ and later on we give a geometrical interpretation of the Chow ring $\mathcal{A}(\mathcal{L}, \mathcal{G})$.

\begin{comment}
We start observing that given a finite lattice $\mathcal{L}$, if $\mathcal{G} = \mathcal{L} \setminus \{\hat{0}\}$ then
\[ \mathcal{N}(\mathcal{L}, \mathcal{L} \setminus \{ \hat{0} \}) = \Delta(\mathcal{L}\setminus \{ \hat{0} \}). \] 
...
\begin{thm}
    Let $\mathcal{L}$ be a finite lattice, and $\mathcal{G} \subset \mathcal{L}$ a building set, then the nested sets complex $\mathcal{N}(\mathcal{L}, \mathcal{G})$ is homotopy equivalent to the order complex $\Delta(\mathcal{L}\setminus \{\ \hat{0} \}).$
Moreover, if $\mathcal{L}$ is atomic, $\mathcal{G}, \mathcal{H} \subset \mathcal{L}$ two building sets such that $\mathcal{H} \subseteq \mathcal{G}$, then the nested sets complex $\mathcal{N}(\mathcal{L}, \mathcal{G})$ is obtained by the nested set complex $\mathcal{N}(\mathcal{L}, \mathcal{H})$ by a sequence of stellar subdivisions. In particular the complexes are homeomorphic.
\end{thm}
\end{comment}

We start by describing a combinatorial version of blowup for a finite meet semilattice.

\begin{defn}
    Let $\mathcal{L}$ be a finite meet semilattice, for an element $X \in \mathcal{L} \setminus \{ \hat{0}\}$, we define a poset $(Bl_X \mathcal{L}, \prec)$ on the set of elements
    \[ Bl_X \mathcal{L} :=\{ Y \, : \, Y \in \mathcal{L}, \, Y \not \ge X \} \cup \{ Y' \, : \, Y \in \mathcal{L}, \, Y \not \ge X, \text{ and $Y \vee X$ exists in $\mathcal{L}$} \}.\]
    The order relation $<$ determine the order $\prec$ on $Bl_X \mathcal{L}$ as
    \[ Y \prec Z, \text{ if $Y < Z$ in $\mathcal{L}$}\]
    \[ Y' \prec Z', \text{ if $Y < Z$ in $\mathcal{L}$}\]
and
    \[Y \prec Z', \text{ if $Y < Z$ in $\mathcal{L}$}\]
    for all $Y, Z \not \ge X$ in $\mathcal{L}$.
    We call $(Bl_X \mathcal{L}, \prec)$ combinatorial blowup of $\mathcal{L}$ in $X$.
\end{defn}

For example.
\begin{es}
    Consider the lattice $\Pi_3$ of partitions of the set $\{1,2,3\}$. We want to compute the combinatorial blowup of 
    $\Pi_3$ in the element $123$, $Bl_{123} \Pi_3$.

    \begin{center} \begin{tikzpicture}
    
        \node[red, draw, fill, circle, scale =0.5,  label =0:123] (top) at (-10,0) {};
        \node[draw, fill, circle, scale =0.5,  label = 180:12] (n1) at (-11,-1.2) {};       
        \node[draw, fill, circle, scale =0.5,  label = 180:13] (n2) at (-10,-1.2) {};
        \node[draw, fill, circle, scale =0.5,  label = 0:23] (n3) at (-9,-1.2) {};

        \node[draw, fill, circle, scale =0.5] (bot) at (-10,-2.4) {};
        ...
        \node[draw, fill, circle, scale =0.5,  label = 180:12] (n4) at (-5,-2) {};       
        \node[draw, fill, circle, scale =0.5,  label = 180:13] (n5) at (-4,-2) {};
        \node[draw, fill, circle, scale =0.5,  label = 0:23] (n6) at (-3,-2) {};

        \node[draw, fill, circle, scale =0.5] (bot2) at (-4,-3.2) {};
        \draw[thick] (bot2) -- (n4);
        \draw[thick] (bot2) -- (n5);
        \draw[thick] (bot2) -- (n6);

        \node[draw, green, fill, circle, scale =0.5,  label = 90:12'] (n7) at (-2,-0.8) {};
        \node[draw, green, fill, circle, scale =0.5,  label = 90:13'] (n8) at (-1,-0.8) {};
        \node[draw, green, fill, circle, scale =0.5,  label = 0:23'] (n9) at (0,-0.8) {};

        \node[draw, red, fill, circle, scale =0.5] (bot3) at (-1,-2) {};
...
We define now the face poset of a set complex which will give us the first interpretations in $\mathcal{N}(\mathcal{L}, \mathcal{G})$.
...
The following theorem shows that the three concept introduced above: combinatorial building sets, nested sets and combinatorial blowups fit toghether providing a combinatorial analogue of the De Concini-Procesi model construction.

\begin{thm}
    Let $\mathcal{L}$ be a lattice, $\mathcal{G}$ a combinatorial building set in $\mathcal{L}$, $G_1, \dots, G_t$ a linear order on $\mathcal{G}$ that is non-increasing with repsect to the partial order on $\mathcal{L}$. Then, consecutive combinatorial blowups in $G_1, \dots, G_t$ resutls in the face poset of the nested set complex $\mathcal{N}(\mathcal{L}, \mathcal{G})$:
    \[ Bl_{G_t}(\dots(Bl_{G_2}(Bl_{G_1}\mathcal{L})) = \mathcal{F}(\mathcal{N}(\mathcal{L}, \mathcal{G}).\]

\end{thm}

Now we investigate the connection between these notions and the stratification in the wonderful model.
We start by defining a notion of building set in the lattice of flats for a linear arrangements of subspaces.

\begin{defn}
    Let $\mathcal{A}$ a arrangements of subspaces in a real or complex vector space $V$, $\mathcal{L}(\mathcal{A})$ the intersection lattice associated to $\mathcal{A}$ and \[cd:\mathcal{L}(\mathcal{A}) \longrightarrow \mathbb{N}\]
    defined as \[ cd(H) := codim_V(H).\]
    A subset $\mathcal{G} \subset \mathcal{L}$ is a geometric building set if it is a building set in the sense of (3.1) and for any $X \in \mathcal{L} \setminus \{ \hat{0} \}$ the codimension of $X$ is equal to the sum of the codimension of its factors
    \[ cd(X) = \sum_{Y \in F_{\mathcal{G}}(X)} cd(Y).\]
\end{defn}

The notion to combinatorial and geometrical building sets are not equivalent but coincide if $\mathcal{A}$ is an arrangements of hyperplanes.
\begin{es}
Let $\mathcal{A}$ be the arrangement of 3 subspaces in $\mathbb{R}^4$
\[ A_1 : x_4 = 0, \, A_2 : x_1 = x_2 = 0, \, A_3 : x_1 = x_3 = 0\]

We depict the intersection lattice $\mathcal{L}(\mathcal{A})$ with its codimension labeling.

\begin{center} \begin{tikzpicture}
    \node[draw, fill, circle, scale =0.5,  label = 180:4] (top) at (0,0) {};

    \node[draw, fill, circle, scale =0.5,  label = 180:3] (n1) at (-2,-1.5) {};
    \node[draw, fill, circle, scale =0.5,  label = 180:3] (n2) at (0,-1.5) {};
    \node[draw, fill, circle, scale =0.5,  label = 180:3] (n3) at (2,-1.5) {};

    \node[draw, fill, circle, scale =0.5,  label = 180:1] (n4) at (-2,-2.5) {};
    \node[draw, fill, circle, scale =0.5,  label = 180:2] (n5) at (0,-2.5) {};
    \node[draw, fill, circle, scale =0.5,  label = 180:3] (n6) at (2,-2.5) {};

    \node[draw, fill, circle, scale =0.5,  label = 180:0] (bot) at (0,-4) {};
...
The set of atoms is a combinatorial building set. However, any geometric building set must contain the intersection $A_2 \cap A_3$ since its codimension is not the sum of codimension of it factors $A_2$ and $A_3$.
\end{es}
...
The blow up version of the model si obtained substituting a linear extension of $\mathcal{L}_{> \hat{0}}$ with a non increasing order on $\mathcal{G}$, and successive blowing up subspaces in $\mathcal{G}$.
...
\begin{thm}
    \begin{enumerate}
        ...
        \item The complement of $\pi^{-1}(\mathcal{M}(\mathcal{A}))$ in $Y_{\mathcal{A}, \mathcal{G}}$ is a divisor with normal crossings; the irreducible components are the proper transforms $D_X$ of intersections $X$ in $\mathcal{G}$,
...
\begin{comment}
Thus given the model $Y_{\mathcal{A}, \mathcal{G}}$, the face poset of its stratification given by irreducible components of divesors and their intersections coincides with the face poset of the nested sets complex. So the nested sets complex encodes the combinatorics of strata in $Y_{\mathcal{A}, \mathcal{G}}$. 
\end{comment}

Summarizing we have that, given $\mathcal{A}$ a linear arrangements of subspaces in a real or complex vector space $V$, then
\begin{enumerate}
    \item[(1)] the lattice $\mathcal{L}(\mathcal{A})$ captures the combinatorics of the stratification of $V$ given by subspaces of $\mathcal{A}$ and their intersections;

    \item[(2)] the nested set complex $\mathcal{N}(\mathcal{L}, \mathcal{G})$ captures the combinatorics of the divisors stratification of the wonderful model $Y_{\mathcal{A}, \mathcal{G}}$.
\end{enumerate}

In addition combinatorial blowups turns out to be the right concept to describe the change in the stratification poset during the successive blow up of subspaces in $\mathcal{G}$. 

\begin{thm}
Let $\mathcal{A}$ be a linear subspace arrangement a real or complex vector space $V$,
$\mathcal{G}$ a geometric building set in $\mathcal{L}(\mathcal{A})$, and $G_1, \dots, G_t$ a non-increasing linear order on $\mathcal{G}$. 
Let $Bl_i(\mathcal{A})$ denote the result of blowing up strata $G_1, \dots, G_i$, for
$i \le t$, and denote by $\mathcal{L}_i$ the face poset of the stratification of $Bl_i(\mathcal{A})$ by proper transforms of subspaces in $\mathcal{A}$ and the exceptional divisors. Then the poset $\mathcal{L}_i$ coincides with the successive combinatorial blowups of $\mathcal{L}$ in $G_1, \dots, G_i$:
\[ \mathcal{L}_i = Bl_{G_i}(\dots(Bl_{G_2}(Bl_{G_1}\mathcal{L}))\dots).\]
\end{thm}

We finish this chapter by giving a geometrical interpretations of the Chow ring $\mathcal{A}(\mathcal{L}, \mathcal{G})$.

Consider a linear essential hyperplanes arrangement $\mathcal{A}$ in a complex vector space $V$. Then, as we already noted, in this case the combinatorical building sets and geometrical building sets are the same.
Considering $\mathcal{G} \subset \mathcal{L}(\mathcal{A})$ a building set, we defined defined the Chow ring $\mathcal{A}(\mathcal{L}, \mathcal{G})$.
...
Then we prove a theorem about the existence of injections and isomorphisms between certain grades of the Feichtner-Yuzvinsky basis. We finish by discussing the hypothesis of the theorem and by giving a counterexample where the theorem fails.
...
\begin{proof}
    ...
    Consider $N^+ = \{F_1, \dots, F_l, F_{l+1} := E \}$, and since $\pi$ and $\lambda$ act similarly we can consider a function $f$ either equal to $\pi$ or $\lambda$. Then $f$ maps a monomial $a = \prod_{i = 1}^{l+1} x_{F_i}^{m_i}$ as follows:
